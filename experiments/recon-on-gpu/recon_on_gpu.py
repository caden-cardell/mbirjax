...
def recon(num_views, num_det_rows, num_det_channels, output_filepath='output.csv'):
    ...
    recon_model.set_params(use_gpu="sinograms")
    time0 = time.time()
    ...
    if not os.path.exists(output_filepath):
        with open(output_filepath, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(['time', 'num_views', 'num_det_rows', 'num_det_channels', 'transfer_pixel_batch_size', 'gpu0_peak_bytes', 'gpu1_peak_bytes', 'gpu2_peak_bytes', 'gpu3_peak_bytes', 'cpu'])

    # append this test data to the output file
    row = [ elapsed, num_views, num_det_rows, num_det_channels, recon_model.transfer_pixel_batch_size ] + [ mem_stats[i]['peak_bytes_in_use'] for i in range(len(mem_stats)) ]
